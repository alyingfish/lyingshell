...
// Wi-Fi detail page: header switch gates the radio; the body lists the top 10
// networks by connection/signal, with an inline password field for unknown
// secured networks. The body is async-incubated by DetailPage.
DetailPage {
    id: root
    ...
    bodyContent: Component {
        Column {
            ...
            // Recomputed imperatively (refresh()) off structural changes and a
            // coarse timer instead of a live sort binding, and fed through
            // ScriptModel so the Repeater reuses row delegates: unchanged
            // networks keep their delegate, re-sorts move them, and only a
            // network leaving the list destroys its row.
            //
            // Reassigning a plain-array model (the previous approach) reset the
            // Repeater and rebuilt ALL 10 rows on every re-sort. A scan-driven
            // re-sort while scrolling then destroyed a row mid MState state-
            // transition (press/hover), and Qt's animation timer later
            // dereferenced the freed delegate (QQuickTransitionManager::complete
            // -> setBinding) -> shell SIGSEGV. Identity-diffing the list keeps
            // the interacted row alive.
            ScriptModel {
                id: wifiModel
            }
            ...
            function refresh() {
                ...
                // ScriptModel diffs by object identity and no-ops on an
                // unchanged list, so reassigning every refresh is cheap.
            ...
            // Re-sort on structural changes only: wifi on/off, the device
            // appearing, or an AP being added/removed (networks.values
            // identity). Reading .values here does NOT subscribe to per-AP
            // signal-strength changes, so scan jitter never re-triggers this;
            // deferred so it runs once after the service singletons settle.
            property int _watch: {
            ...
            // Coarse re-sort cadence: catches APs crossing a signal bar or the
            // scanner dropping/adding networks. ScriptModel no-ops on ticks
            // that change nothing, so this never churns delegates on its own.
            // ponytail: 4s poll; drop it if networks.values gains a per-AP
            // change signal.
            Timer {
                interval: 4000
                running: true
